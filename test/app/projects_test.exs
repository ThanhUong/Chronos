defmodule App.ProjectsTest do
  ...
  describe "projects" do
    ...
    test "create_project/1 with valid data creates a project" do
      ...
      assert {:ok, %Project{} = _project} = Projects.create_project(valid_attrs)
    end
    ...
    test "update_project/2 with valid data updates the project" do
      ...
      assert {:ok, %Project{} = _project} = Projects.update_project(project, update_attrs)
    end
  ...
  describe "project_types" do
    ...
    @valid_attrs %{name: "design"}
    @update_attrs %{name: "corporate"}
    @invalid_attrs %{name: nil}

    test "list_project_types/0 returns all project_types" do
      project_type = project_type_fixture(@valid_attrs)
    ...
    test "create_project_type/1 with valid data creates a project_type" do
      assert {:ok, %ProjectType{} = _project_type} = Projects.create_project_type(@valid_attrs)
    end
    ...
    test "update_project_type/2 with valid data updates the project_type" do
      project_type = project_type_fixture(@valid_attrs)
      assert {:ok, %ProjectType{} = _project_type} = Projects.update_project_type(project_type, @update_attrs)
    end
  ...
  describe "stages" do
    ...
    test "list_stages/0 returns all stages" do
      stage = stage_fixture_with_dependencies()
    ...
    test "create_stage/1 with valid data creates a stage" do
      assert {:ok, %Stage{} = _stage} = Projects.create_stage(valid_stage_attrs())
    end

    test "create_stage/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Projects.create_stage(invalid_stage_attrs())
    end

    test "update_stage/2 with valid data updates the stage" do
      stage = stage_fixture_with_dependencies()
      update_attrs = update_stage_attrs()
      assert {:ok, %Stage{} = _stage} = Projects.update_stage(stage, update_attrs)
    end

    test "update_stage/2 with invalid data returns error changeset" do
      stage = stage_fixture_with_dependencies()
      assert {:error, %Ecto.Changeset{}} = Projects.update_stage(stage, invalid_stage_attrs())
